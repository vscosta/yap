	...
	*current = UNMARK(next);
	...
	POP_CONTINUATION();
     ...
     //      find_ref
        ...
        sz++;
#if DEBUG
	  if (next[sz] != EndSpecials(AbsAppl(next)))  {
...
sweep_trail(choiceptr gc_B, tr_fr_ptr old_TR USES_REGS)
{
  ...
  while (trail_ptr < old_TR) {
	...
	if (marked_ptr)
	  ptr = RepAppl(UNMARK(&TrailTerm(trail_ptr+2)));
	else
