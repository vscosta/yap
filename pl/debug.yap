...
'$trace_goal'(G,M, _Ctx, _GoalNumber, _CP) :-
    '$id_goal'(GoalNumberN),
    '$enter_gate'(call, M:G, GoalNumberN,deterministic),
    (	
   nb_getval(creep,zip)
->
  ...
  true,
	    ...
	    Port,	
         '$at_port'(Port, M:G, GoalNumberN)
    );
    current_choice_point(CP),
    '$predicate_type'(G,M,T),
    catch('$step'(T,M:G,GoalNumberN,CP),
    Error,
    '$trace_error'(Error, GoalNumberN, '$step'(T,M:G,GoalNumberN,CP))
    )).
...
% step a call
% 
'$step'(   exo_procedure,MG,GoalNumber,CP) :-
    !,
    '$step'(   source_procedure,MG,GoalNumber,CP).
'$step'(   mega_procedure,MG,GoalNumber,CP) :-
    !,
    '$step'(   source_procedure,MG,GoalNumber,CP).
'$step'(proxy_procedure,M:G,GoalNumber,CP) :-
    !,
    '$import'(MDonor,M,GDonor,G,_,_),
    '$predicate_type'(GDonor,MDonor,T),
    '$step'(T,MDonor:GDonor,GoalNumber,CP).
'$step'( Type, M:G,GoalNumber,CP0) :-
    % prepare and select matching clause
    Cleanup = '$at_port'(Port, NM:NG, GoalNumber),
    Task0 = cleanup( true, Port, Cleanup, Tag, true, CP0),
    TaskF = cleanup( true, Port, Cleanup, Tag, false, CP0),
    '$setup_call_catcher_cleanup'('$meta_hook'(M:G,NM:NG)),
    '$tag_cleanup'(CP0, Task0),
    (
      Type ==  source_procedure
      ->
      clause(M:G,Body),
      '$trace_goal'(Body,M,inner,GoalNumber, CP0)
      ;
      Type == static_procedure
      ->
      nth_clause(M:Goal,_J,Ref),
      '$creep_clause'( Goal, M, Ref, CP0 )
      ;
    '$execute'(M:G)
    ),
...
'$meta_hook'(MG,M:NG) :-
    ...
    true
    ;
    recorded('$m', meta_predicate(_M,PredDef),_)
    ),
    PredDef=..[N|Ms],
 ...
 *
 */
'$at_port'(Ports, MG, GoalNumber) :-
    '$ports_to_port'(Ports,P),
    ('$deterministic_port'(Ports) -> Deterministic = '?' ; Deterministic = ' '),
    '$enter_gate'(P, MG, GoalNumber,Deterministic).

'$enter_gate'(P, MG, GoalNumber, Deterministic) :-
    '$zip_at_port'(P,GoalNumber,MG),
    !,
    '$action'('\n',P,GoalNumber,MG,Deterministic).  
'$enter_gate'(P, MG, GoalNumber,Deterministic) :-
    '$leap_at_port'(P,GoalNumber,MG),
    !,
    '$action'('\n',P,GoalNumber,MG,Deterministic).  
'$enter_gate'(P, MG, L,Deterministic) :-
    % at this point we are done with leap or skipe
    '$enter_trace'(L, MG, Deterministic),
    repeat,
    '$clear_input'(debugger_input),
    '$trace_msg'(P,MG,L,Deterministic),
    (
      '$has_leash'(P)
      ->
      prompt1(' ? '),
      get_char(user_input,C),
      nb_setval('$spy_start',MG), % where we started from
      '$action'(C,P,L,MG,Deterministic)
      ;
      '$action'('\n',P,L,MG,Deterministic),
      nl(debugger_output)                            
    ),
    !.
...
'$trace_port'(Ports, GoalNumber, MG, Ctx, CP,Info) :-
    '$ports_to_port'(Ports, Port),
    ('$deterministic_port'(Ports,Info) -> true ; true ),
    (
      true
      ;
      '$trace_port_'(Port, GoalNumber, MG, Ctx, CP,Info)
    ).
...
'$ports_to_port'(exception(Exc),exception(Exc)).
'$ports_to_port'(external_exception(Exc),external_exception(Exc)).
...
'$ports_to_port'(     [fail], fail).
'$ports_to_port'(     [fail,none], fail).
%'$ports_to_port'([redo,answer], internal).
...
'$action'('\r',P,CallNumber,Module:G,Deterministic) :-
    !,	% newline
    get_char( debugger_input,C),
    '$action'(C,P,CallNumber,Module:G,Deterministic).
'$action'('\n',_,_CallNumber,_,_) :-
    !,			% newline 	creep
...
'$action'(^,_,_,_: G,_) :- !,			% '
			  ...
			  skip( debugger_input, 10),
			  fail.
'$action'(a,_,_,_,_) :- !,		% 'a		abort
			  skip( debugger_input, 10),
			  nodebug,
			  abort.
'$action'(b,_,_,_,_) :- !,			% 'b		break
			  '$stop_creeping'(_),
			  skip( debugger_input, 10),
			  break,
			  fail.
'$action'('A',_,_,_,_) :- !,			% 'b		break
			    skip( debugger_input, 10),
			    '$stack_dump',
			    fail.
'$action'(c,_,_,_,_) :- !,			% 'c		creep
			  skip( debugger_input, 10),
			  nb_setval(creep,creep),
			  nb_setval('$spy_on',stop),
			  nb_setval('$spy_target',0).
'$action'(e,_,_,_,_) :- !,			% 'e		exit
			  halt.
'$action'(f,_,CallNumber,_,_) :- !,		% 'f		fail
				   '$scan_number'( ScanNumber),
				   ( ScanNumber == 0 -> Goal = CallNumber ; Goal = ScanNumber ),
				   throw(error(debugger_event(fail,Goal),[])).
'$action'(h,_,_,_,_) :- !,			% 'h		help
			  '$action_help',
			  skip( debugger_input, 10),
			  fail.
'$action'(?,_,_,_,_) :- !,			% '?		help
			  ...
			  skip( debugger_input, 10),
			  fail.
'$action'(p,_,_,Module:G,_) :- !,		% 'p		print
				...
				print(user_error,G), nl(user_error)
			       ...
			       skip( debugger_input, 10),
			       fail.
'$action'(d,_,_,Module:G,_) :- !,		% 'd		display
			       ((Module = prolog ; Module = user) ->
...
'$action'(n,_,_,_,_) :-
    ...
    nodebug.
'$action'(r,P,CallNumber,_,_) :- !,	        % r		retry
				   ( '$scan_number'(ScanNumber) -> Goal = ScanNumber ; Goal = CallNumber ),
				   ( (P==call) ->
				     '$ilgl'(s)				%
				     ;
...
'$action'(s,P,CallNumber,_,_) :- !,		% 's		skip
				   ( '$scan_number'(ScanNumber) -> Goal = ScanNumber ; Goal = CallNumber ),
				   ( (P==call; P==redo) ->
				     nb_setval(creep,leap),
				     nb_setval('$spy_on',ignore),
...
'$action'(t,_P,CallNumber,_,_) :-
   ...
   % '$ilgl'(t)				%
   % ).
'$action'(q,P,CallNumber,_,_) :- !,		% qst skip
				   ( '$scan_number'(ScanNumber) -> Goal = ScanNumber ; Goal = CallNumber ),
				   ( (P=call; P=redo) ->
				     nb_setval(creep,leap),
				     nb_setval('$spy_on',stop),
...
'$action'(+,_,_,M:G,_) :- !,			%%		spy this
			  functor(G,F,N), spy(M:(F/N)),
					  skip( debugger_input, 10),
					  fail.
'$action'(-,_,_,M:G,_) :- !,			%% 	nospy this
			  functor(G,F,N),
			  nospy(M:(F/N)),
			  skip( debugger_input, 10),
			  fail.
'$action'(g,_,_,_,_) :- !,			% g		ancestors
			  '$ensure_number'(HowMany),
			  '$show_ancestors'(HowMany),
			  fail.
'$action'('T',exception(G),_,_,_) :- !,	% T		throw
				       throw( G ).
'$action'('L',_,_,_,_) :- !,	% 		throw
			    start_low_level_trace,
			    fail.
'$action'(C,_,_,_,_) :-
    skip( debugger_input, 10),
...
'$action_help' :-
    ...
    format(user_error,'T        throw       L	    low_level_tracer~n', []),
