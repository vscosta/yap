 ...
 **************************************************************************
 *									 *
 * File:		utilities for displaying messages in YAP. * comments:
 *error messages for YAP					 *
 *									 *
...
compose_message( abort(user), _Level) -->
    !,
    [ 'YAP execution aborted' - [] ].
compose_message( loading(_,F), _Level) -->
    { F == user }, !.
...
compose_message(false, _Level) -->
    !,
    [  'no'- []  ].
compose_message(error(E, Info), Level) -->
    structured_message(E, Info, Level).
compose_message(warning(E, Info), Level) -->
    structured_message(E, Info, Level).
compose_message(style_check(What,FILE,Line,Clause), Level)-->
    !,
    { '$show_consult_level'(LC),
      Desc = [prologConsulting=true,
	      parserFile = FILE,
	      parserLine = Line]
    },
    location(style_check(What,FILE,Line,Clause), Desc, Level, LC),
    main_message( style_check(What,FILE,Line,Clause) , Desc, Level, LC ).
...
[0,FileName,LN,0,Level], nl ],
	...
	NLC is LC + 4},
    location_c( E, Desc,true, Level, NLC ) .
location_consult(style_check(A,LN,FileName,B ), _, _Found, Level , LC) -->
    !,
    display_consulting( FileName, Level,style_check(A,LN,FileName,B ),  LC ).
location_consult( E, Desc,Found,Level, LC ) -->
    location_c( E, Desc,Found, Level, LC ) .


location_c( _, Desc, true, Level,  _LC ) -->
    {
...
main_message( style_check(singleton(SVs),_Pos,_File,P),_, _Level, _LC) -->
    ...
    %    {writeln(ci)},
    {
	clause_to_indicator(P, I), (SVs = [_] -> A = variable; A = variables)
    }
    , [ nl, '~*|%% singleton ~s ' - [ 0, A ] ], output_variable_list(SVs),
    [' in ~q.' - [I]].
main_message(style_check(multiple(N, A, Mod, I0), _Pos,
                         File, _P),
             _, _Level, _LC) -->
    !,
    ['~*|%% ~s redefines ~q, originally defined at  ~s.
' -
     [ 0, File, Mod:N / A, I0 ]]
.
main_message(style_check(discontiguous(N, A, Mod), _, _S, _W, _P),_,
             _Level, _LC) -->
    !,
    ['~*|%% discontiguous definition for ~p.' - [ 0, Mod:N / A ]].
main_message(ErrorInfo, _, _Level, _LC) -->
    [nl],
    main_error_message(ErrorInfo),
    [nl].


output_variable_list([V = _]) -->
    !,
    ['~s' - [V]].
output_variable_list([V = _ | SVs]) -->
    ['~s, ' - [V]],
    output_variable_list(SVs).
...
caller(Desc, Level) -->
    {query_exception(errorFunction, Desc, Func), Func \= [],
...
stack_info(Desc, _) -->
    ...
    !, ['~*|Prolog execution stack is:' - [0]], [nl],
    [Stack - []].
stack_info(_, _) -->
    [].
...
system_message(
    error(consistency_error(Who), Where)) -->
...
system_message(error(format_argument_type(Type, Arg), Where)) -->
    ['FORMAT ARGUMENT ERROR- ~~~s called with ~w in ~w: ' -
     [ Type, Arg, Where ]]
.
...
.
system_message(error(existence_error(procedure, P),
                     context(Call, Parent))) -->
    !,
    ['EXISTENCE ERROR- procedure ~w is undefined, called from context  ~w~n                 Goal was ~w' -
     [ P, Parent, Call ]]
.
...
system_message(error(existence_error(Name, F), W)) -->
    ...
    ['EXISTENCE ERROR- ~w could not open ~s ~w' - [ W, ObjName, F ]]
.
system_message(error(evaluation_error(int_overflow), Where)) -->
    ['INTEGER OVERFLOW ERROR- ~w' - [Where]].
system_message(
    error(evaluation_error(float_overflow), Where)) -->
    ['FLOATING POINT OVERFLOW ERROR- ~w' - [Where]].
system_message(
    error(evaluation_error(undefined), Where)) -->
    ['UNDEFINED ARITHMETIC RESULT ERROR- ~w' - [Where]].
system_message(
    error(evaluation_error(underflow), Where)) -->
    ['UNDERFLOW ERROR- ~w' - [Where]].
system_message(
    error(evaluation_error(float_underflow), Where)) -->
    ['FLOATING POINT UNDERFLOW ERROR- ~w' - [Where]].
system_message(
    error(evaluation_error(zero_divisor), Where)) -->
    ['ZERO DIVISOR ERROR- ~w' - [Where]].
system_message(
    error(not_implemented(Type, What), Where)) -->
    ['~w: ~w not implemented- ~w' - [ Where, Type, What ]].
system_message(
    error(operating_SYSTEM_ERROR_INTERNAL, Where)) -->
    ['OPERATING SYSTEM ERROR- ~w' - [Where]].
system_message(
    error(out_of_heap_error, Where)) -->
    ['OUT OF DATABASE SPACE ERROR- ~w' - [Where]].
system_message(
    error(out_of_stack_error, Where)) -->
    ['OUT OF STACK SPACE ERROR- ~w' - [Where]].
system_message(
    error(out_of_trail_error, Where)) -->
    ['OUT OF TRAIL SPACE ERROR- ~w' - [Where]].
system_message(
    error(out_of_attvars_error, Where)) -->
    ['OUT OF STACK SPACE ERROR- ~w' - [Where]].
system_message(
    error(out_of_auxspace_error, Where)) -->
    ['OUT OF AUXILIARY STACK SPACE ERROR- ~w' - [Where]].
system_message(
    error(permission_error(access, private_procedure, P), Where)) -->
    ['PERMISSION ERROR- ~w: cannot see clauses for ~w' - [ Where, P ]]
.
system_message(error(permission_error(access, static_procedure, P),
                     Where)) -->
...
system_message(error(permission_error(create, Name, P), Where)) -->
    ['PERMISSION ERROR- ~w: cannot create ~s ~w' - [ Where, Name, P ]]
.
system_message(error(permission_error(import, M1
                                              : I, redefined, SecondMod),
                     Where)) -->
    ['PERMISSION ERROR- loading ~w: modules ~w and ~w both define ~w' -
     [ Where, M1, SecondMod, I ]]
.
system_message(error(permission_error(input, binary_stream,
                                      Stream),
                     Where)) -->
    ['PERMISSION ERROR- ~w: cannot read from binary stream ~w' -
     [ Where, Stream ]]
.
system_message(error(permission_error(input, closed_stream,
                                      Stream),
                     Where)) -->
    ['PERMISSION ERROR- ~w: trying to read from closed stream ~w' -
     [ Where, Stream ]]
.
system_message(error(permission_error(input, past_end_of_stream,
                                      Stream),
                     Where)) -->
    ['PERMISSION ERROR- ~w: past end of stream ~w' - [ Where, Stream ]]
.
system_message(error(permission_error(input, stream, Stream),
                     Where)) -->
    ['PERMISSION ERROR- ~w: cannot read from ~w' - [ Where, Stream ]]
.
system_message(error(permission_error(input, text_stream, Stream),
                     Where)) -->
    ['PERMISSION ERROR- ~w: cannot read from text stream ~w' -
     [ Where, Stream ]]
.
system_message(error(permission_error(modify, dynamic_procedure,
                                      _),
                     Where)) -->
...
system_message(
    error(permission_error(modify, operator, W), Q)) -->
...
.
system_message(error(permission_error(modify, static_procedure, F),
                     Where)) -->
    ['PERMISSION ERROR- ~w: modifying static procedure ~w' - [ Where, F ]]
.
system_message(error(permission_error(modify,
                                      static_procedure_in_use, _),
                     Where)) -->
...
system_message(
    error(permission_error(module, redefined, Mod), Who)) -->
    ['PERMISSION ERROR ~w- redefining module ~s in a different file' -
     [ Who, Mod ]]
.
system_message(error(permission_error(open, source_sink, Stream),
                     Where)) -->
    ['PERMISSION ERROR- ~w: cannot open file ~w' - [ Where, Stream ]]
.
system_message(error(permission_error(output, binary_stream,
                                      Stream),
                     Where)) -->
    ['PERMISSION ERROR- ~w: cannot write to binary stream ~w' -
     [ Where, Stream ]]
.
system_message(error(permission_error(output, stream, Stream),
                     Where)) -->
    ['PERMISSION ERROR- ~w: cannot write to ~w' - [ Where, Stream ]]
.
system_message(error(permission_error(output, text_stream, Stream),
                     Where)) -->
    ['PERMISSION ERROR- ~w: cannot write to text stream ~w' -
     [ Where, Stream ]]
.
system_message(error(permission_error(resize, array, P),
                     Where)) -->
    ['PERMISSION ERROR- ~w: cannot resize array ~w' - [ Where, P ]]
.
system_message(error(permission_error(unlock, mutex, P),
                     Where)) -->
...
.
system_message(error(representation_error(character_code),
                     Where)) -->
...
system_message(
    error(representation_error(variable), Where)) -->
...
system_message(
    error(resource_error(huge_int), Where)) -->
...
system_message(
    error(resource_error(stack), Where)) -->
    ['RESOURCE ERROR- not enough stack' - [Where]].
system_message(
    error(resource_error(streams), Where)) -->
...
system_message(
    error(resource_error(trail), Where)) -->
    ['RESOURCE ERROR- not enough trail space' - [Where]].
system_message(
    error(signal(SIG, _), _)) -->
    ['UNEXPECTED SIGNAL: ~s' - [SIG]].
...
system_message(error(unhandled_exception,
    ...
    ['UNHANDLED EXCEPTION - message ~w unknown' - [Throw]].
system_message(
    error(uninstantiation_error(TE), _Where)) -->
...
domain_error(array_type,
             Opt) -->
    !,
    ['invalid static array type ~w' - Opt].
domain_error(builtin_procedure,
             _) -->
    !,
...
domain_error(delete_file_option,
             Opt) -->
    !,
...
domain_error(semantics_indicator,
             Opt) -->
    !,
    ['predicate indicator, got ~w' - [Opt]].
domain_error(shift_count_overflow,
             Opt) -->
    !,
...
domain_error(syntax_error_handler,
             Opt) -->
    !,
...
domain_error(thread_create_option,
             Opt) -->
    !,
    ['~w is not a thread_create option' - [Opt]].
domain_error(time_out_spec,
             Opt) -->
    !,
    ['~w is not valid specificatin for time_out' - [Opt]].
domain_error(
    unimplemented_option, Opt) -->
    !,
...
:- !,
    atom_string(A, S), atom_string(B, S1), string_concat([ S,` and `, S1 ], SN)
.
svs([A = _], _, SN)
:- !,
    atom_string(A, S), string_concat(`, and `, S, SN)
.
svs([A = V | L], [A = V | L], SN)
:- !,
    atom_string(A, S), svs(L, [A = V | L], S1), string_concat([ S, S1 ], SN)
.
svs([A = _V | L], All, SN)
:-  !,
    atom_string(A, S), svs(L, All, S1),
...
print_lines_(begin(Severity, OtherKey), S, Prefixes, Key) -->
    !,
    {prefix(Severity, P)}, print_message_lines(S, [P], OtherKey),
...
print_lines_(nl, S, Prefixes, Key) -->
    !,
    {nl(S), Prefixes = [PrefixS - Cmds | More], format(S, PrefixS, Cmds)}, {
	More == [] -> NPrefixes = Prefixes;
	NPrefixes = More
    }
    ,
...
Print a message (see print_message/2) that has been translated location( E, Desc,Level, LC ) -->
    {	query_exception(prologConsulting, Desc, true) },
%    {	query_exception(parserReadingCode, Desc, true)}, 
  ...
  query_exception(parserLine, Desc, LN)
    ...
    [ '~*|~s:~d:~d: ~s: found while consulting' - [0,FileName,LN,0,Level], nl ],
      {NLC is LC+4},
      location_pl( E, Desc,true, Level, NLC ) .

location(style_check(A,LN,FileName,B ),_, Level , LC) -->
  !,
  display_consulting( FileName, Level,style_check(A,LN,FileName,B ),  LC ).

location( E, Desc,Level, LC ) -->
      location_pl( E, Desc,_, Level, LC ) .
...
exceptions, in a human-readable format.
  _Kind_ is one of
`informational`, `banner`, `warning`, `error`, `help` or `silent`.
...
If the Prolog flag verbose is `silent`, messages with
 _Kind_ `informational`, or `banner` are treated as
  silent.
  See `-q` in [Running_YAP_Interactively].


This predicate first translates the  _Term_ into a list of `message
lines` (see print_message_lines/3 for details).
  Next it will
call the hook message_hook/3 to allow the user intercepting the
message.
  If message_hook/3 fails it will print the message unless
 _Kind_ is silent.
...
confusing to YAP (who will process the error?).
 So we write this small
...
query_exception(K,[H|L],V) :-
    lists:memberchk(K=V,[H|L]),
    !.

query_exception(K,exception(E),V) :-
    '$query_exception'(K,E,V).
