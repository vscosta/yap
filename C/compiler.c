...
static int compilerError(yap_error_number id, int drop, Term tm, compiler_struct *cglobs, char *m, ...) {
    yap_error_descriptor_t *e = calloc(1,sizeof(yap_error_descriptor_t));
    Yap_MkErrorRecord(e, __FILE__, __FUNCTION__, __LINE__, id, tm, m);
    Term t = Yap_MkFullError(e);
    Yap_PrintWarning(t);
    if (drop) {
/* oops, too deep a term */
        save_machine_regs();
        LOCAL_Error_Size = 0;
        siglongjmp(cglobs->cint.CompilerBotch, drop);
    } else {
        c_goal(Yap_MkApplTerm(FunctorCall,1, &tm), CurrentModule, cglobs);
    }
    return 0;
}
...
static void compile_sf_term(Term t, int argno, int level) {
  ...
  while ((argno = *++p)) {
    ...
    if (t != nullvalue) {
     ...
     compilerError(SYSTEM_ERROR_COMPILER, 0, t, "illegal argument of soft-functor);
      } else
...
inline static void c_args(Term app, unsigned int level,
    ...
    if (Arity >= MaxTemps) {
        compilerError(SYSTEM_ERROR_COMPILER, OUT_OF_AUX_BOTCH, app, cglobs, "exceed maximum arity of compiled goal");
    }
...
static int try_store_as_dbterm(Term t, Int argno, unsigned int arity, int level,
    ...
    while ((g = Yap_SizeGroundTerm(t, TRUE)) < 0) {
        compilerError(RESOURCE_ERROR_AUXILIARY_STACK, OUT_OF_AUX_BOTCH, t, cglobs, "aux ed goal");
    }
    // if (g < 16)
    return FALSE;
...
static void c_test(Int Op, Term t1, compiler_struct *cglobs) {
    ...
    if (Op == _save_by) {
        if (!IsNewVar(t)) {
            char s[32];
            Yap_bip_name(Op, s);
            compilerError(UNINSTANTIATION_ERROR, true, t, cglobs, "compiling %s/2 on bound variable", s);
        }
    ...
    if (IsVarTerm(t1)) {
        if (IsVarTerm(t2)) {
            ...
            if (Op == _arg) {
                ...
                char s[32];
                Yap_bip_name(Op, s);
                compilerError(INSTANTIATION_ERROR, 0, Goal, cglobs, "compiling %s/2 with output bound", s);
            }
        }
    } else { /* t1 is bound */
        /* it has to be either an integer or a floating point */
        if (IsVarTerm(t2)) {
            if (IsNewVar(t2)) {
                char s[32];
                compilerError(INSTANTIATION_ERROR, 0, Goal, cglobs, "compiling %s/3", s);
            }
        } else {
            if (Op == _functor) {
                ...
                if (!IsIntegerTerm(t2)) {
                    compilerError(TYPE_ERROR_INTEGER, 0, Goal, cglobs, "compiling functor/3");
                }
                i2 = IntegerOfTerm(t2);
                if (i2 < 0) {
                    compilerError(DOMAIN_ERROR_NOT_LESS_THAN_ZERO, 0, Goal, cglobs, "compiling functor/3");
                ...
                if (IsNumTerm(t1)) {
                    ...
                    compilerError(TYPE_ERROR_ATOM, 0, Goal, cglobs, "compiling functor/3");
                }
                if (i2 == 0)
                    c_eq(t1, t3, cglobs);
                else {
                    ...
                    if (t1 == TermDot && i2 == 2) {
                        if (HR + 2 >= (CELL *) cglobs->cint.freep0) {
                            /* oops, too many new variables */
                            compilerError(TYPE_ERROR_ATOM, OUT_OF_TEMPS_BOTCH, Goal, cglobs, "compiling functor/3");
                        }
                        RESET_VARIABLE(HR);
                        RESET_VARIABLE(HR + 1);
                        HR += 2;
                        ...
                        *HR++ = (CELL) Yap_MkFunctor(AtomOfTerm(t1), i2);
                        ...
                        if (EndOfPAEntr(p0)) {
                            compilerError(TYPE_ERROR_ATOM, OUT_OF_HEAP_BOTCH, Goal, cglobs, "compiling functor/3");
                        }
                ...
                if (IsIntegerTerm(t1))
                    ...
                    compilerError(TYPE_ERROR_INTEGER, 0, Goal, cglobs, "compiling arg");
                }
                if (IsAtomicTerm(t2) ||
                    (IsApplTerm(t2) && IsExtensionFunctor(FunctorOfTerm(t2)))) {
                    compilerError(TYPE_ERROR_COMPOUND, 0, Goal, cglobs, "compiling arg");
        ...
        if (Op == _functor) {
            if (!IsAtomicTerm(t1)) {
                compilerError(TYPE_ERROR_ATOM, 0, Goal, cglobs, "compiling arg");
            } else {
                if (!IsVarTerm(t2)) {
                    Int arity;
...
yamop *Yap_cclause(volatile Term inp_clause, Int NOfArgs, Term mod,
    ...
    if (FunctorOfTerm(my_clause) == FunctorAssert) {
        head = ArgOfTerm(1, my_clause);
        body = ArgOfTerm(2, my_clause);
    } else {
        head = my_clause, body = MkAtomTerm(AtomTrue);
    }
    if (!Yap_callable(head)) {
        compilerError(TYPE_ERROR_CALLABLE, 0, head,& cglobs, "clause head should be atom or compound term, head");
        return (0);
    } else {
        loop:
