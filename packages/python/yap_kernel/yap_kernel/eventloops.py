...
def _notify_stream_qt(kernel, stream):
    ...
    from yap_ipython.external.qt_for_kernel import QtCore
...
def register_integration(*toolkitnames):
    """Decorator to register an event loop to integrate with the yap_ipython kernel
    ...
    The decorated function should take a single argument, the yap_ipython kernel
...
def _loop_qt(app):
    ...
    Pulled from guisupport.start_event_loop in yap_ipython < 5.2,
    since yap_ipython 5.2 only checks `get_ipython().active_eventloop` is defined,
...
def loop_qt4(kernel):
    ...
    from yap_ipython.lib.guisupport import get_app_qt4
    ...
    if kernel is None:
        ...
        if kernel is None:
            raise RuntimeError("You didn't specify a kernel,"
                " and no yap_ipython Application with a kernel appears to be running."
            )
    loop = loop_map[gui]
