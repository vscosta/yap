...
static Term CopyTermToArena(Term t, bool share, bool copy_att_vars, UInt arity,
  ...
  while (true) {
    ...
    CELL *pf = HR, *HLow = pf;
    HR++;
    if ((res = copy_complex_term(ap - 1, ap, share, copy_att_vars, &pf, bindp,
                                 arenap, &cspace, stt PASS_REGS)) == 0) {
      // goto error_handler;
      ...
      if (arenap) {
        size_t min_grow =
            (stt->restarts < 16 ? 16 * 1024 * stt->restarts * stt->restarts
                                : 128 * 1024);
        *arenap = CloseArena(&cspace PASS_REGS);
        if ((*arenap = GrowArena(*arenap, min_grow, 1, &cspace PASS_REGS)) ==
            0) {
          close_stack(stt);
          Yap_ThrowError(RESOURCE_ERROR_STACK, TermNil, LOCAL_ErrorMessage);
          return 0L;
...
Term Yap_TermAsForest(Term t1, Term *list) /* copy term t to a new instance  */
{
  *list = TermNil;
...
static Int p_nb_set_shared_arg(USES_REGS1) {
  ...
  COPY(ARG3);
  to = CopyTermToArena(
      Deref(ARG3), TRUE, TRUE, 3, &LOCAL_GlobalArena, NULL,
      garena_overflow_size(ArenaPt(LOCAL_GlobalArena) PASS_REGS) PASS_REGS);
  if (to == 0L)
    return FALSE;
  dest = Deref(ARG2);
...
void Yap_InitGlobals(void) {
  ...
  */
  ...
  /** @pred  copy_term(? _TI_,- _TF_) is iso
      ...
      + suspended goals and attributes for attributed variables in _TI_ are
     also duplicated;
