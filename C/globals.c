...
static inline Term exit_cell_space(cell_space_t *cs) {
    Term o = TermNil;
  if (cs->arenaL) {
      o = CreateNewArena(HR, ASP - HR);
      HR = cs->oH;
  }
    HB = cs->oHB;
    ASP = cs->oASP;
  ...
  if (size < 4*MIN_ARENA_SIZE) {
   ...
   if (pt == HR) {
      HR += size;
        ...
        XREGS[arity + 1] = arena;
        if ((size = Yap_InsertInGlobal(pt, size * sizeof(CELL))/sizeof(CELL)) == 0) {
        return false;
      }
        arena = XREGS[arity + 1];
     }
...
static Term CopyTermToArena(Term t, bool share, bool copy_att_vars,
...
#endif
        if (share && VarOfTerm(t) > ArenaPt(*arenap)) {
	  Term ar = CloseArena(&cspace PASS_REGS);
	  if (arenap)
	    *arenap = ar;
            return t;
    ...
    switch (res) {
        case -1:
            ...
            HR = HB;
	    Term ar =  CloseArena(&cspace PASS_REGS);
            if((ar=GrowArena(ar, min_grow, arity + 1, &cspace PASS_REGS))==0) {
                Yap_ThrowError(RESOURCE_ERROR_STACK, TermNil, LOCAL_ErrorMessage);
                return 0L;
            }
	    if (arenap)
...
{
  Term t = CopyTermToArena(ARG1, false,  TRUE, 2, NULL, 0 PASS_REGS);
  if (t == 0L)
    return FALSE;
...
static Term CreateTermInArena( Atom Na, UInt Nar, UInt arity,
...
static Int p_nb_setarg(USES_REGS1) {
  ...
  to = Deref(ARG3);
  to = CopyTermToArena(
      ARG3, FALSE, TRUE, 3, &LOCAL_GlobalArena,
...
static Int p_nb_set_shared_arg(USES_REGS1) {
  ...
  if (pos < 1 || pos > arity)
    return FALSE;
  to = CopyTermToArena(
      ARG3, TRUE, TRUE, 3, &LOCAL_GlobalArena,
      garena_overflow_size(ArenaPt(LOCAL_GlobalArena) PASS_REGS) PASS_REGS);
  if (to == 0L)
    return FALSE;
  dest = Deref(ARG2);
...
static Int p_nb_create_accumulator(USES_REGS1) {
  ...
  to = CopyTermToArena(
      t, TRUE, TRUE, 2, &LOCAL_GlobalArena,
      garena_overflow_size(ArenaPt(LOCAL_GlobalArena) PASS_REGS) PASS_REGS);
  if (to == 0L)
    return FALSE;
  t2 = Deref(ARG2);
  ...
  if (IsIntegerTerm(t0) && IsIntegerTerm(tadd)) {
    ...
    if (IsIntTerm(new)) {
      ...
      if (IsLongIntTerm(t0)) {
        ...
        new = CopyTermToArena(
            new, TRUE, TRUE, 2, &LOCAL_GlobalArena,
...
Term Yap_SetGlobalVal(Atom at, Term t0) {
  ...
  to = CopyTermToArena(
      t0, FALSE, TRUE, 2, &LOCAL_GlobalArena,
      garena_overflow_size(ArenaPt(LOCAL_GlobalArena) PASS_REGS) PASS_REGS);
  if (to == 0L)
    return to;
  WRITE_LOCK(ge->GRWLock);
...
Term Yap_SaveTerm(Term t0) {
  ...
  Term to;
  to = CopyTermToArena(
		       Deref(t0), FALSE, TRUE, 2, &LOCAL_GlobalArena,
      garena_overflow_size(ArenaPt(LOCAL_GlobalArena) PASS_REGS) PASS_REGS);
  if (to == 0L)
    return to;
  return to;
...
static Int p_nb_set_shared_val(USES_REGS1) {
  ...
  ge = GetGlobalEntry(AtomOfTerm(t) PASS_REGS);
  to = CopyTermToArena(
      ARG2, TRUE, TRUE, 2, &LOCAL_GlobalArena,
      garena_overflow_size(ArenaPt(LOCAL_GlobalArena) PASS_REGS) PASS_REGS);
  if (to == 0L)
    return FALSE;
  WRITE_LOCK(ge->GRWLock);
...
static Int p_nb_create(USES_REGS1) {
  ...
  to = CreateTermInArena(AtomOfTerm(tname),
...
static Int p_nb_queue_enqueue(USES_REGS1) {
  ...
  to = CopyTermToArena(ARG2, FALSE, TRUE, 2, &arena,
                       min_size PASS_REGS);
    if (to == 0L) {
      return FALSE;
    }
    ...
    *HR++ = to;
    RESET_VARIABLE(HR);
    HR++;
    Term nto = AbsPair(HR-2);
    if (qsize == 0) {
      qd[QUEUE_HEAD] = nto;
    } else {
      *VarOfTerm(qd[QUEUE_TAIL]) = nto;
  }
    qd[QUEUE_TAIL] = (CELL)(HR-1);
  qd[QUEUE_ARENA] = CloseArena(&cspace);
 return TRUE;
}
...
static Int p_nb_heap_add_to_heap(USES_REGS1) {
  ...
  qd = GetHeap(ARG1, "add_to_heap");
   ...
   pt = qd + HEAP_START;
    pt[2 * hsize] = to;
  to = CopyTermToArena(ARG3, FALSE, TRUE, 3, &arena,
                       mingrow PASS_REGS);
...
hsize = new_beam_entry(qd);
  ...
  if (arena == 0L)
    return FALSE;
  mingrow = garena_overflow_size(ArenaPt(arena) PASS_REGS);
  key = CopyTermToArena(ARG2,FALSE, TRUE, 3, &arena,
                        mingrow PASS_REGS);
  to = CopyTermToArena(ARG3, FALSE, TRUE, 3, &arena,
                       mingrow PASS_REGS);
  if (key == 0 || to == 0L)
