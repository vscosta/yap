...
static inline void free_request(YAP_UInt v, bool valid) {
...
static YAP_Bool mpi_irecv(void) {
  ...
  CONT_TIMER();
  char *tmp = malloc(sz);
  YAP_UInt r = new_request(NULL, tmp);
...
static YAP_Bool mpi_test(void) {
  ...
  free_request(YAP_IntOfTerm(t1), status.MPI_ERROR != MPI_ERR_BUFFER);
  PAUSE_TIMER();
  return (YAP_Unify(t2, YAP_MkIntTerm(status.MPI_ERROR)));
}
