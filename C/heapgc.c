...
static tr_fr_ptr
sweep_regs(int num_regs, tr_fr_ptr old_TR, yamop *nextop USES_REGS)
{
  tr_fr_ptr oTR = TR;
  TR = old_TR;
  push_registers(num_regs, sweep, nextop PASS_REGS);
  TR = oTR;
  return old_TR;
  }
#undef PUSH
...
static void
pop_registers(Int num_regs, tr_fr_ptr old_TR, yamop *nextop USES_REGS)
{
  ...
  tr_fr_ptr trnow = TR;
  TR = old_TR;
  push_registers(num_regs, pop,nextop PASS_REGS);
  TR = trnow;
}
  ...
  if (IsVarTerm(ccur)) {
	...
	*current = UNMARK_CELL(cnext);
	...
	if (next[sz-1] != CloseExtension(next))  {
	  fprintf(stderr,"%s:%s:b%d [ Error: could not find ES for blob at %p:\n functor %lx,\n type=%lx,\n size=%ld,\n end=%p,\n terminator=%lx, \n HR=%p -- ASP=%p ]\n",__FILE__,__FUNCTION__,__LINE__, next, next[0],next[1],sz,next+sz,next[sz-1], HR, ASP);
	}
#endif
