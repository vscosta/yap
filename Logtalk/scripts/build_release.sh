...
cd ..
cp -R logtalk/manuals man2302
tar -czf man2302.tgz man2302
mv logtalk lgt2302
...
cd $dir
dpkg-deb --build debian logtalk_2.30.2-1_all.deb

md5="`md5 -q lgt2302.tar.bz2`"
sudo mkdir -p /opt/local/var/db/dports/distfiles/logtalk
sudo cp -f lgt2302.tar.bz2 /opt/local/var/db/dports/distfiles/logtalk/lgt2302.tar.bz2
...
sudo sed -e 's/^version.*/version 2.30.2/' -i '' Portfile
sudo sed -e "s/^checksums.*/checksums md5 $md5/" -i '' Portfile
sudo sed -e 's/^distname.*/distname lgt2302/' -i '' Portfile
...
cp -R work/logtalk-2.30.2.pkg $dir
...
cd man2302/userman
./userman.sh
mv userman.pdf ../../manpdf2302
cd ../refman
./refman.sh
mv refman.pdf ../../manpdf2302
cd ../..
tar -czf manpdf2302.tgz manpdf2302
