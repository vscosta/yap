...
static Term visitor_error_handler( Ystack_t *stt, CELL *hb, CELL *asp,
	  ...
	  CELL *end;
            ...
            if (arenap && *arenap) {
                ...
                if (sz0 < sz + MIN_ARENA_SIZE) {
                    ...
                    size_t nsize = Yap_InsertInGlobal(end, (sz + 4 * MIN_ARENA_SIZE) * CellSize, &end) / CellSize;
                    if (bindp)
                    ...
                    while (HR + (MIN_ARENA_SIZE + sz) > ASP) {
                        ...
                        if (bindp)
                            yt1 = Yap_InitHandle(*bindp);
                        if (!Yap_dogcl(2 * MIN_ARENA_SIZE + sz PASS_REGS)) {
                             stt->err = RESOURCE_ERROR_STACK;
                            break;
                        }
                        if (bindp)
...
rational_tree_to_forest(USES_REGS1) /* copy term t to a new instance  */
{
    ...
    COPY(ARG1);
    Term t = CopyTermToArena(Deref(ARG1), true, false, NULL, &list PASS_REGS);
    if (t == 0L)
        return FALSE;
    /* be careful, there may be a stack shift here */
...
{
    Term list = TermNil;
    Term t = CopyTermToArena(t1, true, false, NULL, &list PASS_REGS);
    if (t == 0L)
        return FALSE;
    /* be careful, there may be a stack shift here */
    Term ts[2];
