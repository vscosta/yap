    ...
    if (val == TermAtom) {
        ...
        m->flags |= SNGQ_CODES;
            ...
            m->flags |= SNGQ_CHARS;
                ...
                Yap_ThrowError(DOMAIN_ERROR_PROLOG_FLAG, val,
...
static Int yap_flag(USES_REGS1) {
 ...
 tflag = Yap_StripModule(tflag, &CurrentModule);
  ...
  if (IsVarTerm(val)) {
    ...
    if (flag == 0)
      return false;
    return Yap_unify(flag, val);
  } else {
    Yap_set_flag(tflag, val);
  }
  return true;
}
...
do_prolog_flag_property(Term tflag,
  ...
  if (!fv)
    return false;
  //Yap_ThrowError(DOMAIN_ERROR_PROLOG_FLAG,MkAtomTerm(atflag),NULL);
