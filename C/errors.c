...
/**
   ...
   - Yap_UserError(): try to discriminate between a throw and an system error;
...
bool Yap_MkErrorRecord(yap_error_descriptor_t *r, const char *file,
  ...
  if (where == 0L || where == TermNil) {
    ...
    if (where) {
      r->culprit = Yap_TermToBuffer(where, Quote_illegal_f | Handle_vars_f |
                                               Handle_cyclics_f);
    }
  }
...
static yap_error_descriptor_t *mkUserError(Term t, Term *tp,
