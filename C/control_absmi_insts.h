...
#ifdef INDENT_CODE
{
 ...
 SET_ASP(YREG, AS_CELLS(PREG->y_u.s.s));
    ...
    NoStackCommitY:
      ...
      BEGD(d0);
      d0 = XREG(PREG->y_u.xps.x);
      deref_head(d0, soft_cut_b_x_unk);
    soft_cut_b_x_nvar:
      /* skip a void call and a label */
      SET_ASP(YREG, AS_CELLS(PREG->y_u.xps.s));
      PREG = NEXTOP(NEXTOP(NEXTOP(PREG, xps),Osbpp),l);
...
#endif /* YAPOR_SBA && FROZEN_STACKS */
         if (pt0==B) {
   while(pt0->cp_b && pt0->cp_b->cp_ap == TRUSTFAILCODE) {
     pt0=pt0->cp_b;
   }
 saveregs();
        prune(pt0 PASS_REGS);
   ...
   pt0->cp_ap = TRUSTFAILCODE;
      ...
      BEGP(pt1);
      deref_body(d0, pt1, soft_cut_b_x_unk, soft_cut_b_x_nvar);
      ENDP(pt1);
      /* never cut to a variable */
      /* Abort */
      FAIL();
    ...
    NoStackSoft_CutX:
      ...
      Op(soft_cut_b_y, yps);
      CACHE_Y_AS_ENV(YREG);
      check_stack(NoStackSoft_CutY, HR);
      ENDCACHE_Y_AS_ENV();
      BEGD(d0);
      d0 = YREG[PREG->y_u.yps.y];
      deref_head(d0, soft_cut_b_y_unk);
    soft_cut_b_y_nvar:
      SET_ASP(YREG, AS_CELLS(PREG->y_u.yps.s));
      PREG = NEXTOP(NEXTOP(NEXTOP(PREG, yps),Osbpp),l);
      {
 ...
 saveregs();
   ...
   pt0->cp_ap = TRUSTFAILCODE;
      ...
      BEGP(pt1);
      deref_body(d0, pt1, soft_cut_b_y_unk, soft_cut_b_y_nvar);
      ENDP(pt1);
      /* never cut to a variable */
      /* Abort */
