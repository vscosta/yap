# -*- coding: utf-8 -*-
"""
Sphinx directive to support embedded yap_ipython code.

This directive allows pasting of entire interactive yap_ipython sessions, prompts
...
and the yap_ipython directive::

    extensions = ['yap_ipython.sphinxext.ipython_console_highlighting',
                  'yap_ipython.sphinxext.ipython_directive']

The yap_ipython directive outputs code-blocks with the language 'ipython'. So
if you do not have the syntax highlighting extension enabled as well, then
all rendered code-blocks will be uncolored. By default this directive assumes
that your prompts are unchanged yap_ipython ones, but this can be customized.
...
ipython_rgxin:
    The compiled regular expression to denote the start of yap_ipython input
    lines. The default is re.compile('In \[(\d+)\]:\s?(.*)\s*'). You
    shouldn't need to change this.
ipython_rgxout:
    The compiled regular expression to denote the start of yap_ipython output
    lines. The default is re.compile('Out\[(\d+)\]:\s?(.*)\s*'). You
    shouldn't need to change this.
ipython_promptin:
    The string to represent the yap_ipython input prompt in the generated ReST.
    The default is 'In [%d]:'. This expects that the line numbers are used
    in the prompt.
ipython_promptout:
    The string to represent the yap_ipython prompt in the generated ReST. The
...
ipython_mplbackend:
    ...
    used. To use the yap_ipython directive without matplotlib as a dependency, set
...
As an example, to use the yap_ipython directive when `matplotlib` is not available,
...
from yap_ipython import InteractiveShell
...
class EmbeddedSphinxShell(object):
    """An embedded yap_ipython instance to run inside Sphinx"""
    ...
    def __init__(self, exec_lines=None):
        ...
        # Create config object for yap_ipython
        config = Config()
        ...
        # Store a few parts of yap_ipython we'll need.
        self.IP = IP
    ...
    def ensure_pyplot(self):
        """
        Ensures that pyplot has been imported into the embedded yap_ipython shell.
...
class IPythonDirective(Directive):
    ...
    def setup(self):
        ...
        if self.shell is None:
            ...
            # Store yap_ipython directive to enable better error messages
    ...
    def run(self):
        ...
        if len(lines) > 2:
            if debug:
                ...
                # these lines out, then no yap_ipython code will appear in the
                # final output.
