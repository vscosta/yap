...
static bool should_creep() {
  return !(LOCAL_PrologMode &
           (AbortMode | InterruptMode | SystemMode | BootMode)) &&
...
inline static bool CallMetaCall(Term t, Term mod USES_REGS) {
  ...
  if (IsIntTerm(t) || (IsApplTerm(t) && IsExtensionFunctor(FunctorOfTerm(t))))
    Yap_ThrowError(TYPE_ERROR_CALLABLE, Yap_TermToIndicator(t, mod),
                   "meta-call");
  Term tmod;
  if (mod) {
    tmod = mod;
...
Term Yap_ExecuteCallMetaCall(Term g, Term mod) {
  ...
  if (IsIntTerm(g) || (IsApplTerm(g) && IsExtensionFunctor(FunctorOfTerm(g))))
    Yap_ThrowError(TYPE_ERROR_CALLABLE, Yap_TermToIndicator(g, mod),
                   "meta-call");
  ts[0] = g;
...
static bool EnterCreepMode(Term t, Term mod USES_REGS) {
  ...
  PredCreep =
      Yap_get_pred(Yap_MkNewApplTerm(FunctorCreep, 1), CurrentModule, "creep");
  PP = PredCreep;
  if (IsVarTerm(t)) {
    return false;
  }
...
bool Yap_execute_pred(PredEntry *ppe, CELL *pt, bool pass_ex USES_REGS) {
  ...
  if (out) {
    ...
    // We'll pass it through
    if (Yap_HasException()) {
      if (pass_ex && ((LOCAL_PrologMode & BootMode) || !CurrentModule)) {
...
void Yap_InitExecFs(void) {
  ...
  Yap_InitComma();
  Yap_InitCPred("restore_regs", 1, restore_regs, NoTracePredFlag);
  Yap_InitCPred("restore_regs", 2, restore_regs2, NoTracePredFlag);
...
#ifdef DEPTH_LIMIT
  Yap_InitCPred("$execute_under_depth_limit", 2, execute_depth_limit,
                NoTracePredFlag);
#endif
  ...
  Yap_InitCPred("$current_choice_point", 1, current_choice_point,
                NoTracePredFlag);
  Yap_InitCPred("$current_choicepoint", 1, current_choice_point,
                NoTracePredFlag);
