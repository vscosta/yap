...
add2strings(innerpage(_,_),Strings,S-Sb-Sd-Sa-Sg-Sp-Sl,S-Sb-Sd-[Strings|Sa]-Sg-Sp-Sl) :-
    !.
add2strings(location(_,_),Strings,S-Sb-Sd-Sa-Sg-Sp-Sl,S-Sb-Sd-Sa-Sg-Sp-[Strings|Sl]) :-
    !.
...
process(_,location(_Atts,Children)) -->
    { key_in(file(F),Children) },
    { key_in(line(L),Children)->string_concat(":",L,LInfo) ; LInfo="" },
   !,
    { format(string(S), ' <small> generated by YAPDocs from ~s~s <small/>~n'  , [F,LInfo]) },
[S].
...
parameternamelist(parameternamelist([parametername(_,[Name])]))  -->
 ["  ",Name].
parameternamelist(parameterdescription(_,Para)) -->
  foldl(para,Para),
["\n"].
