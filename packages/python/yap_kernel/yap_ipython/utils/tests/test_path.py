...
from yap_ipython.testing.decorators import (skip_if_not_win32, skip_win32,
                                        onlyif_unicode_paths,)
from yap_ipython.testing.tools import make_tempfile, AssertPrints
from yap_ipython.utils import path
...
def setup_environment():
    ...
    oldstuff = (env.copy(), os.name, sys.platform, path.get_home_dir, yap_ipython.__file__, os.getcwd())

def teardown_environment():
    """Restore things that were remembered by the setup_environment function
    """
    (oldenv, os.name, sys.platform, path.get_home_dir, yap_ipython.__file__, old_wd) = oldstuff
    os.chdir(old_wd)
    reload(path)
...
def test_get_home_dir_1():
    ...
    #fake filename for yap_ipython.__init__
    yap_ipython.__file__ = abspath(join(HOME_TEST_DIR, "Lib/yap_ipython/__init__.py"))
...
def test_get_home_dir_2():
    ...
    sys.frozen = True
    #fake filename for yap_ipython.__init__
    yap_ipython.__file__ = abspath(join(HOME_TEST_DIR, "Library.zip/yap_ipython/__init__.py")).lower()
