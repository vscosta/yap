...
from yap_ipython.utils.tokenutil import token_at_cursor, line_at_cursor
...
class YAPKernel(KernelBase):
    ...
    use_experimental_completions = Bool(True,
        help="Set this flag to False to deactivate the use of experimental YAP completion APIs.",
    ).tag(config=True)
    ...
    def _restore_input(self):
        ...
        if PY3:
            ...
            builtin_mod.input = self._sys_input
    ...
    def _experimental_do_complete(self, code, cursor_pos):
        """
        Experimental completions from YAP, using Jedi.
        """
        if cursor_pos is None:
            cursor_pos = len(code)
...
class Kernel(YAPKernel):
    def __init__(self, *args, **kwargs):
        import warnings
        warnings.warn('Kernel is a deprecated alias of yap_kernel.ipkernel.YAPKernel',
                      DeprecationWarning)
        super(YAPKernel, self).__init__(*args, **kwargs)
