...
static Int
p_stream_flags ( USES_REGS1 )
{				/* '$stream_flags'(+N,-Flags)            */
  Term trm;
  trm = Deref (ARG1);
  if (IsVarTerm (trm) || !IsIntTerm (trm))
    return (FALSE);
  return (Yap_unify_constant (ARG2, MkIntTerm (GLOBAL_Stream[IntOfTerm (trm)].status)));
}



static Int
p_check_stream (USES_REGS1)
...
static Int
p_check_if_stream (USES_REGS1)
{				/* '$check_stream'(Stream)                  */
  int sno = Yap_CheckStream (ARG1, Input_Stream_f | Output_Stream_f | Append_Stream_f | Socket_Stream_f,  "check_stream/1");
  if (sno != -1)
    UNLOCK(GLOBAL_Stream[sno].streamlock);
  return sno != -1;
}
...
{				/* '$set_output'(+Stream,-ErrorMessage)  */
 ...
 char *
 Yap_guessFileName(int f, int sno, char *nameb, size_t max)
{
#if __linux__
  char path[256];
  if (snprintf(path, 255, "/proc/self/fd/~d",f) &&
      readlink( path, nameb, max) )
    return nameb;
#elif __APPLE__
...
#elif __WIN32_
  FILE_NAME_INFO *fni = (FILE_NAME_INFO *)malloc(sizeof(FILE_NAME_INFO)+sizeof(WCHAR)*MAXPATHLEN);
...
static Int
representation_error ( int sno, Term t2 USES_REGS )
{
...
static Int
file_name ( int sno, Term t2 USES_REGS )
{
  char s[MAXPATHLEN+1];
...
static Int
file_no ( int sno, Term t2 USES_REGS )
{
  int f = Yap_GetStreamFd(sno);
  Term rc = MkIntTerm(f) ;
...
static bool
SetCloseOnAbort( int sno, bool close )
{
  if (close) {
...
static Int
has_close_on_abort ( int sno, Term t2 USES_REGS )
{				/* '$set_output'(+Stream,-ErrorMessage)  */
...
static bool
SetBuffering ( int sno, Atom at )
{				/* '$set_bufferingt'(+Stream,-ErrorMessage)  */
  if (at == AtomFull) {
    ...
    if (setvbuf( GLOBAL_Stream[sno].file, NULL, _IOLBF, 0) < 0)
      return PlIOError( SYSTEM_ERROR, Yap_MkStream( sno ), "could not set line buffering");
...
#define    STREAM_PROPERTY_DEFS() \
    ...
    PAR( "representation_errors", filler, STREAM_PROPERTY_REPRESENTATION_ERRORS ), \
...
#define PAR(x,y,z) { x , y, z }
 ...
 static const param_t stream_property_defs[] =
  {
    STREAM_PROPERTY_DEFS()
  };
#undef PAR
...
{				/* Init current_stream */
  ...
  for (i=0; i < STREAM_PROPERTY_END; i ++) {
	...
	rc = rc && 
	  has_close_on_abort ( sno, args[STREAM_PROPERTY_CLOSE_ON_ABORT].tvalue PASS_REGS);
	break;
	...
	rc = rc && 
	  found_eof ( sno, args[STREAM_PROPERTY_END_OF_STREAM].tvalue PASS_REGS);
	break;
	...
	rc = rc && 
	  file_no ( sno, args[STREAM_PROPERTY_FILE_NO].tvalue PASS_REGS);
	break;
      case STREAM_PROPERTY_INPUT:
	rc = rc && 
	  is_input ( sno PASS_REGS);
	break;
      case STREAM_PROPERTY_MODE:
	rc = rc && 
	  stream_mode  ( sno, args[STREAM_PROPERTY_MODE].tvalue   PASS_REGS);
	break;
      case STREAM_PROPERTY_OUTPUT:
	rc = rc && 
	  is_output ( sno PASS_REGS);
	break;
	...
	rc = rc && 
	 representation_error  ( sno, args[STREAM_PROPERTY_REPRESENTATION_ERRORS].tvalue   PASS_REGS);
	break;
      case STREAM_PROPERTY_TYPE:
	rc = rc && 
	 stream_type  ( sno, args[STREAM_PROPERTY_TYPE].tvalue   PASS_REGS);
	break;
      case STREAM_PROPERTY_TTY:
	rc = rc && 
	 stream_tty  ( sno, args[STREAM_PROPERTY_TTY].tvalue   PASS_REGS);
 	break;
...
#define PAR(x,y,z) { x , y, z }
 ...
 static const param_t set_stream_defs[] =
  {
    SET_STREAM_DEFS()
  };
#undef PAR
...
static bool
do_set_stream (int sno, Term opts USES_REGS)
{				/* Init current_stream */
  xarg *args;
  ...
  for (i=0; i < SET_STREAM_END; i ++) {
	...
	rc = rc &&
	  Yap_AddAlias ( AtomOfTerm(t), sno );
	break;
      case SET_STREAM_BUFFER:
	rc = rc &&
	  SetBuffering ( sno, AtomOfTerm(t) );
	break;
      case SET_STREAM_BUFFER_SIZE:
	rc = rc &&
	  SetBuffer ( sno, IntegerOfTerm(t) );
	break;
	...
	break;
	 ...
	 } else if (t2 == TermReset) {GLOBAL_Stream[sno].status |= 
	   GLOBAL_Stream[sno].status &= ~Eof_Error_Stream_f;
	   GLOBAL_Stream[sno].status |= Reset_Eof_Stream_f;	   
	...
	break;
	 ...
	 } else if (t2 == TermError) {GLOBAL_Stream[sno].status |= 
	   GLOBAL_Stream[sno].status &= ~RepError_Xml_f;
	   GLOBAL_Stream[sno].status |=  RepError_Prolog_f;	   
	 } else if (t2 == TermEOfCode) {
	   GLOBAL_Stream[sno].status &= ~RepError_Xml_f;
	 ...
	 rc &= 
	 stream_type  ( sno, args[SET_STREAM_TYPE].tvalue   PASS_REGS);
	break;
      case SET_STREAM_TTY:
	rc &= 
	 stream_tty  ( sno, args[SET_STREAM_TTY].tvalue   PASS_REGS);
 	break;
      case SET_STREAM_END:
	rc = false;
	break;
...
static Int
set_stream (USES_REGS1)
{				/* Init current_stream */
  int sno = Yap_CheckStream (ARG1, Input_Stream_f | Output_Stream_f | Append_Stream_f, "set_stream_position/2");
  if (sno < 0) {
    return (FALSE);
  }
...
{
    CACHE_REGS
  if (!(GLOBAL_Stream[sno].status & (Null_Stream_f|Socket_Stream_f|InMemory_Stream_f|Pipe_Stream_f)))
    fclose (GLOBAL_Stream[sno].file);
#if HAVE_SOCKET
  else if (GLOBAL_Stream[sno].status & (Socket_Stream_f)) {
    Yap_CloseSocket(GLOBAL_Stream[sno].u.socket.fd,
 ...
 */
static Int
set_input ( USES_REGS1 )
{				/* '$show_stream_position'(+Stream,Pos) */
  int sno =
    Yap_CheckStream (ARG1, Input_Stream_f, "set_input/1");
  if (sno < 0)
    return false;
...
static Int
p_file_name ( USES_REGS1 )
{
  Term tout;
  int sno = Yap_CheckStream (ARG1, Input_Stream_f | Output_Stream_f | Append_Stream_f,"file_name/2");
  if (sno < 0)
    return (FALSE);
#if HAVE_SOCKET
...
static Int
p_cur_line_no ( USES_REGS1 )
{				/* '$current_line_number'(+Stream,-N) */
  Term tout;
  int sno =
  Yap_CheckStream (ARG1, Input_Stream_f | Output_Stream_f | Append_Stream_f,"current_line_number/2");
  if (sno < 0)
    return (FALSE);
  ...
  if (GLOBAL_Stream[sno].status & Tty_Stream_f)
	...
	{
	  if (!(GLOBAL_Stream[i].status & (Free_Stream_f|Socket_Stream_f|Pipe_Stream_f|InMemory_Stream_f)) &&
...
static Int
p_line_position ( USES_REGS1 )
{				/* '$line_position'(+Stream,-N) */
  Term tout;
  int sno = Yap_CheckStream (ARG1, Input_Stream_f | Output_Stream_f | Append_Stream_f, "line_position/2");
  if (sno < 0)
    return (FALSE);
  if (GLOBAL_Stream[sno].status & Tty_Stream_f)
    {
...
static Int
p_character_count ( USES_REGS1 )
{				/* '$character_count'(+Stream,-N) */
  Term tout;
  int sno =  Yap_CheckStream (ARG1, Input_Stream_f | Output_Stream_f | Append_Stream_f, "character_count/2");
  if (sno < 0)
    return (FALSE);
  if (GLOBAL_Stream[sno].status & Tty_Stream_f)
    {
...
static Int
p_show_stream_flags( USES_REGS1 )
{				/* '$show_stream_flags'(+Stream,Pos) */
  Term tout;
  int sno =
    Yap_CheckStream (ARG1, Input_Stream_f | Output_Stream_f | Append_Stream_f, "stream_property/2");
  if (sno < 0)
    return (FALSE);
...
static Int
p_show_stream_position ( USES_REGS1 )
{				/* '$show_stream_position'(+Stream,Pos) */
  Term tout;
  int sno =
    Yap_CheckStream (ARG1, Input_Stream_f | Output_Stream_f | Append_Stream_f, "stream_position/2");
  if (sno < 0)
    return (FALSE);
...
static Int
set_stream_position ( USES_REGS1 )
