...
setof(Template, Generator, Set) :-
    nb:nb_heap(1024, H),
    % Template is the output, Key unquantified variables
    '$free_variables_in_term'(Template^Generator, StrippedGenerator, QueryVars),
    terms:new_variables_in_term(Template,QueryVars,Key),
    (
	StrippedGenerator,
	nb:nb_heap_add(H,Key-Template,[]),
	fail
	% solutions should be stored
    ;
        collect(H,Key,InternalSet)
    ),
     must_be_of_type( list_or_partial_list, Set ),
Set = InternalSet.
...
collect(H,Key,Bag)  :-
           ...
           nb:nb_heap_del(H,Key-V,[]   ),
           samek(H,Key,Vs),
           sort([V|Vs],Bag),
            ( nb:nb_heap_empty(H)
          ->
          !, % make determinate when we have no solutions
           nb:nb_heap_close(H)
           ;
           true
        ).
...
% ot just take the first one, but also
% generate the bags
samek( H, K, Set) :-
    nb:nb_heap_peek(H,Key-V,[]),
	terms:variant(K,Key),
	!,
	Key = K, 
	nb:nb_heap_del(H,K-_,_n   ),
    Set= [V|Vs],
    samek( H, K, Vs).
samek( _H, _K, []).
...
bagof(Template, Generator, Bag) :-
     must_be_of_type( list_or_partial_list, Bag ),
     '$bagof'(Template, Generator, Bag).
