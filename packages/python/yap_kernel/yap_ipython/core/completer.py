...
for normal classic Python code, as well as completer for yap_ipython specific
Syntax like magics.

Latex and Unicode completion
============================

yap_ipython and compatible frontends not only can complete your code, but can help
...
yap_ipython, or any compatible frontend you can prepend backslash to the character
...
Starting with yap_ipython 6.0, this module can make use of the Jedi library to
...
module in debug mode (start yap_ipython with ``--Completer.debug=True``) in order
to have extra logging information is :any:`jedi` is crashing, or if current
yap_ipython completer pending deprecations are returning results not yet handled
by :any:`jedi`
...
from yap_ipython.core.inputsplitter import ESC_MAGIC
from yap_ipython.core.latex_symbols import latex_symbols, reverse_latex_symbol
from yap_ipython.core.oinspect import InspectColors
from yap_ipython.utils import generics
from yap_ipython.utils.dir2 import dir2, get_real_method
...
class _FakeJediCompletion:
    ...
    Added in yap_ipython 6.0 so should likely be removed for 7.0
...
class Completion:
    ...
    ``yap_ipython.python_matches``, ``yap_ipython.magics_matches``...).
    """
    ...
    def __init__(self, start: int, end: int, text: str, *, type: str=None, _origin='', signature='') -> None:
        warnings.warn("``Completion`` is a provisional API (as of yap_ipython 6.0). "
...
def _deduplicate_completions(text: str, completions: _IC)-> _IC:
    ...
    the yap_ipython completer does return things that Jedi does not, but should be
    at some point.
    """
...
def rectify_completions(text: str, completions: _IC, *, _debug=False)->_IC:
    ...
    completion are return by the yap_ipython completer and not found in Jedi in
    order to make upstream bug report.
    """
    warnings.warn("`rectify_completions` is a provisional API (as of yap_ipython 6.0). "
                 "It may change without warnings. "
                 "Use in corresponding context manager.",
                  category=ProvisionalCompleterWarning, stacklevel=2)
    ...
    if diff and _debug:
        print('yap_ipython.python matches have extras:', diff)
...
class CompletionSplitter(object):
    ...
    # yap_ipython's most typical usage patterns.
    _delims = DELIMS
...
class IPCompleter(Completer):
    ...
    def _limit_to_all_changed(self, change):
        warnings.warn('`yap_ipython.core.IPCompleter.limit_to__all__` configuration '
            'value has been deprecated since yap_ipython 5.0, will be made to have '
            'no effects and then removed in future version of yap_ipython.',
            UserWarning)
    ...
    def __init__(self, shell=None, namespace=None, global_namespace=None,
        ...
        if use_readline is not _deprecation_readline_sentinel:
            warnings.warn('The `use_readline` parameter is deprecated and ignored since yap_ipython 6.0.',
    ...
    def completions(self, text: str, offset: int)->Iterator[Completion]:
        ...
        """
        warnings.warn("_complete is a provisional API (as of yap_ipython 6.0). "
                      "It may change without warnings. "
                      "Use in corresponding context manager.",
                      category=ProvisionalCompleterWarning, stacklevel=2)
    ...
    def complete(self, text=None, line_buffer=None, cursor_pos=None):
        ...
        warnings.warn('`Completer.complete` is pending deprecation since '
                'yap_ipython 6.0 and will be replaced by `Completer.completions`.',
