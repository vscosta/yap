 ...
 * in which the goal is to be called.
 * A Query is either open or closed: when closed, it has no connection to the Prolog system;
 * when open, it is linked to an active goal within a Prolog engine.
 ...
 * For example, if p(a) and p(b) are facts in the Prolog
 * database, then the following is equivalent to printing all
 * the solutions to the Prolog query p(X):
 * <pre>
...
public class Query implements Enumeration {
	 ...
	 * Calling close() on an already closed Query is harmless (has no effect).
	 * 
	 ...
	 * If the return value is null, this means that the Query has no solutions.
	 */
