...
static inline UInt ArenaSzW(Term arena) { return big2arena_szW(RepAppl(arena)); }
...
static Term NewArena(size_t sizeW, arity_t arity, CELL *where, CELL *top, int wid) {
    ...
    if (where == HR)
        where = NULL;
    if ((new_sizeB = Yap_InsertInGlobal(where, sizeW * CellSize, &at)) == 0) {
...
static Int p_allocate_arena(USES_REGS1) {
    ...
    return Yap_unify(ARG2, NewArena(IntegerOfTerm(t), 1, NULL, NULL, worker_id));
}
...
void Yap_AllocateDefaultArena(size_t gsizeW, int wid, void *cs) {
    REMOTE_GlobalArena(wid) = NewArena(gsizeW, 2, NULL, NULL, wid);
}
...
static Term GrowArena(Term arena, size_t sizeW, UInt arity,
                      cell_space_t *cspace USES_REGS) {
    CELL *pt, *pt2, **at;
    size_t old_sizeW = sizeW;
    ...
    while (true) {
        ...
        if (sizeW == 0) {
            sizeW = (MIN_ARENA_SIZE << (3 + cspace->restarts));
        }
        if (sizeW < K << cspace->restarts) sizeW = K << (cspace->restarts + 1);
        if (sizeW > K * K) sizeW = K * K;
           pt = ArenaLimit(arena);
    size_t sizeB = sizeW * sizeof(CELL), sizeB0 = sizeB;
    sizeB = Yap_InsertInGlobal(pt, sizeB, &pt);
    printf(" %ld--%ld\n", sizeB, sizeB0);
    if (sizeB < sizeB0) {
        continue;
    }
    sizeW = sizeB / sizeof(CELL);
    arena = CreateNewArena(pt - old_sizeW, pt + sizeW);
    return arena;
...
CELL *Yap_GetFromArena(Term *arenap, size_t cells, UInt arity) {
    ...
    size_t old_szW = ArenaSzW(arena);
    if (base + cells > max - MIN_ARENA_SIZE) {
        size_t extra_sizeW = MIN_ARENA_SIZE * 64;
        if ((extra_sizeW = Yap_InsertInGlobal(max, extra_sizeW * 2 * sizeof(CELL), &base) / sizeof(CELL)) == 0) {
            Yap_ThrowError(RESOURCE_ERROR_STACK, TermNil,
                           "No Stack Space for Non-Backtrackable terms");
        }
        old_szW += extra_sizeW;
    ...
    if (arenap) {
        *arenap = CreateNewArena(base, max PASS_REGS);
    }
    return base;
}
                    ...
                    switch (dd1) {
                        ...
                        default: {
                            ...
                            if (ptd1[1] == BIG_RATIONAL) {
                                ...
                                szW = (2 * sizeof(MP_INT) + 3 * CellSize +
                                       ((MP_INT *) (ptd1 + 2))->_mp_alloc * sizeof(mp_limb_t)) /
                                      CellSize + (((MP_INT *) (ptd1 + 2) + 1)->_mp_alloc * sizeof(mp_limb_t)) /
                                                 CellSize;
                            } else {
...
bool Yap_visitor_error_handler(Ystack_t *stt, void *cs_) {
    ...
    if (stt->bindp) {
        ctb = Yap_InitHandleWithAddress(stt->bindp);
    }
    if (LOCAL_Error_TYPE == RESOURCE_ERROR_AUXILIARY_STACK) {
        ...
        if (cs) {
            ...
            if (*stt->arenap && (stt->arenap = GrowArena(*stt->arenap, min_grow, 1, cs PASS_REGS)) != NULL) {
                cs->oH = HR;
                cs->oHB = HB;
            }
        }
    ...
    if (stt->bindp) {
        stt->bindp = Yap_PopHandleWithAddress(ctb);
    }
...
static Term CopyTermToArena(Term t, bool share, bool copy_att_vars,
                            Term **arenapp, Term *bindp, CELL *startp,
                            size_t min_grow USES_REGS) {
    struct cell_space cspace;
...
static Int nb_setarg(USES_REGS1) {
    ...
    to = Deref(ARG3);
    CELL *loc_globp = &LOCAL_GlobalArena;
    to = CopyTermToArena(
            Deref(ARG3), FALSE, TRUE, &loc_globp, NULL, ArenaPt(LOCAL_GlobalArena),
...
static Int nb_set_shared_arg(USES_REGS1) {
    ...
    if (pos < 1 || pos > arity)
        return FALSE;
    COPY(ARG3);
    CELL *local_globp = &LOCAL_GlobalArena;
    to = CopyTermToArena(
            Deref(ARG3), TRUE, TRUE, &local_globp, NULL, ArenaPt(LOCAL_GlobalArena),
            garena_overflow_size(LOCAL_GlobalArena PASS_REGS) PASS_REGS);
    if (to == 0L)
        return FALSE;
    dest = Deref(ARG2);
...
static Int nb_create_accumulator(USES_REGS1) {
    ...
    CELL *local_globp = &LOCAL_GlobalArena;
    to = CopyTermToArena(
            t, TRUE, TRUE, &local_globp, NULL, ArenaPt(LOCAL_GlobalArena),
            garena_overflow_size(LOCAL_GlobalArena PASS_REGS) PASS_REGS);
    if (to == 0L)
        return FALSE;
    t2 = Deref(ARG2);
...
static Int nb_add_to_accumulator(USES_REGS1) {
    ...
    if (IsIntegerTerm(t0) && IsIntegerTerm(tadd)) {
        ...
        if (IsIntTerm(new)) {
            ...
            if (IsLongIntTerm(t0)) {
                ...
                COPY(new);
                CELL *local_globp = &LOCAL_GlobalArena;
                new = CopyTermToArena(new, TRUE, TRUE, &local_globp, NULL, ArenaPt(LOCAL_GlobalArena),
    ...
    if (IsNumTerm(t0) && IsNumTerm(tadd)) {
        ...
        CELL *local_globp = &LOCAL_GlobalArena;
        new = CopyTermToArena(
                new, TRUE, TRUE, &local_globp, NULL, ArenaPt(LOCAL_GlobalArena),
                garena_overflow_size(LOCAL_GlobalArena PASS_REGS) PASS_REGS);
        destp = RepAppl(Deref(ARG1));
        destp[1] = new;
...
Term Yap_SetGlobalVal(Atom at, Term t0) {
    ...
    CELL *local_globp = &LOCAL_GlobalArena;
    to = CopyTermToArena(
            t0, FALSE, TRUE, &local_globp, NULL, ArenaPt(LOCAL_GlobalArena),
            garena_overflow_size(LOCAL_GlobalArena PASS_REGS) PASS_REGS);
    if (to == 0L)
        return to;
    WRITE_LOCK(ge->GRWLock);
...
Term Yap_CopyTermToArena(Term inp, Term *arenap) {
    CACHE_REGS
    CELL **arenapp = &arenap;
    return CopyTermToArena(inp, false, true, arenapp, NULL, arenap, 0 PASS_REGS);
}
...
Term Yap_SaveTerm(Term t0) {
    ...
    CELL *local_globp = &LOCAL_GlobalArena;
    to = CopyTermToArena(
            Deref(t0), FALSE, TRUE, &local_globp, NULL, ArenaPt(LOCAL_GlobalArena),
            garena_overflow_size(LOCAL_GlobalArena PASS_REGS) PASS_REGS);
    if (to == 0L)
        return to;
    return to;
...
static Int nb_set_shared_val(USES_REGS1) {
    ...
    CELL *local_globp = &LOCAL_GlobalArena;
    to = CopyTermToArena(
            ARG2, TRUE, TRUE, &local_globp, NULL, ArenaPt(LOCAL_GlobalArena),
            garena_overflow_size(LOCAL_GlobalArena PASS_REGS) PASS_REGS);
    if (to == 0L)
        return FALSE;
    WRITE_LOCK(ge->GRWLock);
...
static Int nb_queue_close(USES_REGS1) {
    ...
    if (!IsVarTerm(t)) {
        ...
        qp[QUEUE_HEAD] = qp[QUEUE_TAIL] = TermNil;
...
static Int nb_queue_enqueue(USES_REGS1) {
    ...
    CELL *arenap = &arena;
    qd[QUEUE_ARENA] = TermNil;
    to = CopyTermToArena(t, false, true, &arenap, NULL, qd,
                         min_size PASS_REGS);
    qd = GetQueue(ARG1, "enqueue");
    qd[QUEUE_SIZE] = MkIntTerm(qsize + 1);
    qd[QUEUE_ARENA] = *arenap;
    /* garbage collection ? */
...
static Term MkZeroApplTerm(Atom f, UInt sz, CELL *arena, UInt arena_sz,
                           int wid USES_REGS) {
    Term t0, tf;
    CELL *pt;
    ...
    *HR = (CELL) fsz;
    ...
    HR = pt;
    *arena = NewArena(arena_sz, 0, NULL, NULL, wid);
    return tf;
}
...
static Int nb_heap(USES_REGS1) {
    ...
    Term heap = MkZeroApplTerm(AtomHeapData, 2 * hsize + HEAP_START + 1, &heap_arena,
                               arena_sz, worker_id);
    if (heap != TermNil) {

        CELL *ar = RepAppl(heap) + 1;
        ar[HEAP_ARENA] = heap_arena;
...
static Int nb_beam_add_to_beam(USES_REGS1) {
    ...
    Term l = MkPairTerm(ARG2, ARG3);
    mingrow = garena_overflow_size(arena PASS_REGS);
    to = CopyTermToArena(l, FALSE, TRUE, &arenap, NULL, qd, mingrow PASS_REGS);
    if (to == 0)
        return FALSE;
