...
setup_engine(int myworker_id, int init_thread)
{
 ...
 REMOTE_ThreadHandle(myworker_id).tgoal =  Yap_FetchTermFromDB(Yap_StoreTermInDB(REMOTE_ThreadHandle(myworker_id).tgoal) );
 REMOTE_ThreadHandle(myworker_id).texit =  Yap_FetchTermFromDB(  Yap_StoreTermInDB(REMOTE_ThreadHandle(myworker_id).texit) );
...
void Yap_InitThreadPreds(void)
{ 
  Yap_InitCPred("c_with_mutex", 2, p_with_mutex, MetaPredFlag);
  Yap_InitCPred("mutex_create", 1, p_new_mutex, SafePredFlag);
  Yap_InitCPred("$max_workers", 1, p_max_workers, 0);
  Yap_InitCPred("$thread_self", 1, p_thread_self, SafePredFlag);
