...
%%
% @pred $file_loaded(FileName, CurrentModule, InnerModule)
%
%   
% we cannot consult the file we are consulting:
'$file_loaded'(FileName, TargetModule, TargetModule) :-
    '__NB_getval__'('$consulting_file', FileName, fail),
    !.
% if the file exports a module, then we can
% be imported from any module.
'$file_loaded'(F, _TargetModule, M) :-
    % make sure: it either defines a new module or it was loaded in the same context
    '$module'(F,M,_P,_),
    !.
% otherwise, we can be reused if we were defined for the same module.
'$file_loaded'(F, TargetModule, TargetModule) :-
    recorded('$lf_loaded','$lf_loaded'( F, TargetModule, _Reconsult, _UserFile, _OldF, _Line, _Opts), _).
