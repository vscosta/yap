...
from yapkernel.control import CONTROL_THREAD_NAME
from yapkernel.jsonutil import json_clean
from yapkernel.kernelbase import Kernel
...
class Comm(BaseComm, traitlets.config.LoggingConfigurable):
    ...
    kernel = Instance("yapkernel.kernelbase.Kernel", allow_none=True)  # type:ignore[assignment]
    comm_id = Unicode()
    ...
    def __init__(
        ...
        if show_warning:
            warn(
                "The `yapkernel.comm.Comm` class has been deprecated. Please use the `comm` module instead."
