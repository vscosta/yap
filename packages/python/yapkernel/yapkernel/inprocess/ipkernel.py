...
from yapkernel.jsonutil import json_clean
from traitlets import Any, Enum, Instance, List, Type, default
from yapkernel.ipkernel import IPythonKernel
from yapkernel.zmqshell import ZMQInteractiveShell
...
class InProcessKernel(IPythonKernel):
    ...
    frontends = List(
        Instance('yapkernel.inprocess.client.InProcessKernelClient',
                 allow_none=True)
    )
...
class InProcessInteractiveShell(ZMQInteractiveShell):
    ...
    kernel = Instance('yapkernel.inprocess.ipkernel.InProcessKernel',
                      allow_none=True)
    ...
    def enable_gui(self, gui=None):
        ...
        from yapkernel.eventloops import enable_gui
        if not gui:
            gui = self.kernel.gui
