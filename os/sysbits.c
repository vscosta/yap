...
/// File Error Handler
static void Yap_FileError(yap_error_number type, Term where, const char *format,
                          ...) {
...
#endif /* MACYAP */
static char *expandVars(const char *spec);
...
static bool exists(const char *f) { return has_access(f, F_OK); }
...
static char *PlExpandVars(const char *source) {
...
// s = getenv("HOMEPATH");
      ...
      if ((user_passwd = getpwnam(result)) == NULL) {
        Yap_FileError(SYSTEM_ERROR_OPERATING_SYSTEM,
...
const char *DirName(const char *X) {
  ...
  if ((err = _splitpath_s(o, drive, YAP_FILENAME_MAX - 1, dir,
    ...
    Yap_FileError(SYSTEM_ERROR_OPERATING_SYSTEM, TermNil,
...
static const char *expandWithPrefix(const char *source, const char *root) {
  char *work;
...
/** Yap_trueFileName: tries to generate the true name of  file
   ...
   * @return
   */
bool Yap_trueFileName(const char *isource, const char *idef, const char *iroot,
                      char *result, bool access, file_type_t ftype,
                      bool expand_root, bool in_lib) {
      ...
      if (pt) {
        source =
            (ftype == YAP_SAVED_STATE || ftype == YAP_OBJ ? "../../lib/Yap"
                                                          : "../../share/Yap");
        if (Yap_trueFileName(source, NULL, pt, save_buffer, access, ftype,
                             expand_root, in_lib))
          root = save_buffer;
        else
          done = true;
...
int Yap_TrueFileName(const char *source, char *result, bool in_lib) {
  return Yap_trueFileName(source, NULL, NULL, result, true, YAP_PL, true,
                          in_lib);
}

int Yap_TruePrefixedFileName(const char *source, const char *root, char *result,
                             int in_lib) {
  return Yap_trueFileName(source, NULL, root, result, true, YAP_PL, true,
                          in_lib);
}
...
static Int p_expand_file_name(USES_REGS1) {
  ...
  if (!Yap_trueFileName(RepAtom(AtomOfTerm(t))->StrOfAE, NULL, NULL,
                        LOCAL_FileNameBuf, true, YAP_PL, true, false))
    return false;
  return Yap_unify(ARG2, MkAtomTerm(Yap_LookupAtom(LOCAL_FileNameBuf)));
}
...
static Int true_file_name3(USES_REGS1) {
  ...
  if (!Yap_trueFileName(RepAtom(AtomOfTerm(t))->StrOfAE, NULL, root,
