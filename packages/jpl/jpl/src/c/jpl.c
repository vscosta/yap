...
jni_hr_del(
	...
	for ( epp=&(hr_table->slots[index]), ep=*epp ; ep!=NULL ; epp=&(ep->next), ep=*epp )
		...
		*epp = ep->next;				/* bypass the entry */
		...
		DEBUG(1, Sdprintf( "[found & removed hashtable entry for object reference %p]\n", (void *)iref));
		return TRUE;					/* entry found and removed */
		}
	    }
	}
    DEBUG(1, Sdprintf("[JPL: failed to find hashtable entry for (presumably bogus) object reference %p]\n", (void *)iref));
    return FALSE;
    }
...
JNIEXPORT void JNICALL
 Java_jpl_fli_Prolog_close_1query(
	...
	qid_t		qid;
        ...
        DEBUG(1, Sdprintf( ">close_query(env=%lu,jProlog=%lu,jquid=%lu)...\n", (long)env, (long)jProlog, (long)jqid));
    if	(   jpl_ensure_pvm_init(env)
...
JNIEXPORT jint JNICALL													/* returns -1, 0 or 1 (or -2 for error) */
 Java_jpl_fli_Prolog_compare(
	...
	if	(	jpl_ensure_pvm_init(env)
		...
		{
		DEBUG(1, Sdprintf( "> PL_compare( %lu, %lu)", term1, term2));
 ...
 Java_jpl_fli_Prolog_exception(
	...
	return	(	jpl_ensure_pvm_init(env)
		 ...
		 /* &&	jqid != NULL	// redundant */
			...
			&&	( DEBUG(1, Sdprintf( "	ok: setTermTValue(env,term_t,term)\n")), TRUE )
			?	(
					DEBUG(1, Sdprintf("  =%lu\n",(long)term_t)),
					term_t
				)
...
JNIEXPORT void JNICALL
 Java_jpl_fli_Prolog_unregister_1atom(
	...
	DEBUG(1, Sdprintf( ">unregister_atom(env=%lu,jProlog=%lu,jatom=%lu)...\n", (long)env, (long)jProlog, (long)jatom));
