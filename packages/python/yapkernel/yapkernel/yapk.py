...
class YAPRun(InteractiveShell):
    ...
    def prolog_call(self,howmany, ccell, result):
        ...
        try:
            if self.iterations:
                self.result.result = self.answers
            else:
                self.result.result = []
                print("No\n")
            return self.result
    ...
    def prolog(self, ccell, result):
        ...
        try:
            ...
            if not query.isspace():
                ...
                self.q.port = "call"
                self.q.answer = None
                return self.prolog_call(iterations, ccell, self.result)
            else:
                self.result.result = []
                return self.result
        except Exception as e:
...
# run the new command using the given tracer
    ...
    def error_before_exec(self,value):
        ...
        self.last_execution_result = self.result
        return self.result
    ...
    def yrun_cell(
        ...
        *,
        ...
        self.result = ExecutionResult(info)
        ...
        if self.store_history:
            self.result.execution_count = self.execution_count
      ...
      # Store raw and processed history
        ...
        if self.q and self.os == (program,squery):
            return self.prolog_call(iterations, cell, self.result)
        # new cell
        self.os = None
        _run_async = False
        ...
        self.displayhook.exec_result = self.result
        ...
        has_raised =  self.prolog(cell ,self.result)
