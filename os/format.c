...
static Int doformat(volatile Term otail, volatile Term oargs,
    ...
    if (setjmp(format_botch)) {
        ...
        *HR++ = otail;
        ...
        oargs = HR[-2];
        otail = HR[-1];
        GLOBAL_Stream[sno].u.mem_string.pos = finfo->old_pos;
        HR -= 2;
    }
    ...
    while ((fptr += get_utf8(fptr, -1, &ch)) && ch) {
        ...
        switch (ch) {
            ...
            case 'c': {
                ...
                if (IsVarTerm(t)) {
                    format_clean_up(sno, sno0, finfo);
                    Yap_ThrowError(INSTANTIATION_ERROR, targs[targ], "command %c in format %s", ch, fstr);
                }
                if (!IsIntegerTerm(t)) {
                    format_clean_up(sno, sno0, finfo);
                ...
                if (nch < 0) {
                    format_clean_up(sno, sno0, finfo);
                    Yap_ThrowError(DOMAIN_ERROR_NOT_LESS_THAN_ZERO, targs[targ], "command %c in format %s", ch,
                                   fstr);
                }
                if (!has_repeats)
                    repeats = 1;
            ...
            case 'G': {
                ...
                if (!IsNumTerm(t)) {
                    format_clean_up(sno, sno0, finfo);
                    Yap_ThrowError(TYPE_ERROR_NUMBER, targs[targ], "command %c in format %s", ch, fstr);
                }
                if (IsIntegerTerm(t)) {
