...
PyObject *PythonLookup(const char *s, PyObject *oo) {
  ...
  if ((o = lookupPySymbol(s, oo, NULL)) == NULL)
    ...
    yt = TailOfTerm(yt);
    YAP_PutInSlot(t, yt);
    ctx = term_to_python(hd, eval, ctx, false);
  }
  ...
  while (IsApplTerm(yt)  && FunctorOfTerm(yt) == (Functor)FUNCTOR_sqbrackets2) {
    ...
    // get the context
    ctx = yap_to_python(tail, eval, ctx, false);
    // t now refers to the index
    index = ArgOfTerm(1, yt);
    index = HeadOfTerm(index);
    yt = TailOfTerm(yt);
    i = yap_to_python(index, true, NULL, false);
    // check numeric
...
static PyObject *bip_int(term_t t) {
  ...
  static PyObject *bip_sum(term_t t) {
    ...
    */
	...
	if (PL_is_atom(t)  && !(o= find_obj(context, NULL, t, false)))
	  return o;
	...
	if (PL_is_atom(t) && (o = find_obj(context, NULL, t, true)))
	  return o;
	AOK(PL_get_name_arity(t, &name, &arity), NULL);
	if (arity == 0) {
	...
	if (!PL_get_functor(t, &fun))
	  return NULL;
	if ((o=find_obj(context, NULL, t, true)))
	  return o;
