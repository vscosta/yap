...
#define JMPNext()             \
  if (ExpEnv.config_struc.current_displacement)       \
    JMP((void *) OpAddress[Yap_op_from_opcode(PREG->opc) + ExpEnv.config_struc.current_displacement]); \
  JMP((void *) (PREG->opc));

#define JMPNextW()              \
  if (ExpEnv.config_struc.current_displacement)       \
    JMP((void *) OpAddress[Yap_op_from_opcode(PREG->y_u.o.opcw) + ExpEnv.config_struc.current_displacement]); \
...
#define SUCCESSBACK()             \
  {                 \
    if (Yap_op_from_opcode((*_PREG)->opc) == _jit_handler) {    \
      if ((*_PREG)->y_u.jhc.jh->caa.naddress != -1 && NativeArea->area.p[(*_PREG)->y_u.jhc.jh->caa.naddress] && NativeArea->area.ok[(*_PREG)->y_u.jhc.jh->caa.naddress]) { \
  void* (*callee)(yamop**,yamop**,CELL**,void*[],void*[]);  \
  void* go;             \
  callee = (void*(*)(yamop**,yamop**,CELL**,void*[],void*[]))NativeArea->area.p[(*_PREG)->y_u.jhc.jh->caa.naddress]; \
  go = (*callee)(&(*_PREG), &(*_CPREG), &(*_SREG), external_labels, OpAddress); \
  return go;              \
      }                 \
    }                 \
    if ((char*)ExpEnv.debug_struc.pprint_me.nativerun_exit_by_success != 0 && (char*)ExpEnv.debug_struc.pprint_me.nativerun_exit_by_success != (char*)0x1) { \
      fprintf(stderr, "%s:%d\n", __FILE__, __LINE__);     \
      fprintf(stderr, "%s", (char*)ExpEnv.debug_struc.pprint_me.nativerun_exit_by_success); \
    }                 \
    HEADPREG->y_u.jhc.jh->jitman.torecomp = ExpEnv.config_struc.torecompile; \
    NativeArea->success[HEADPREG->y_u.jhc.jh->caa.naddress] += 1;   \
    (ExpEnv.config_struc.torecompile) ? (ExpEnv.config_struc.current_displacement = ExpEnv.config_struc.TOTAL_OF_OPCODES) \
...
#define BACK()                \
  {                 \
   if ((char*)ExpEnv.debug_struc.pprint_me.nativerun_exit_by_fail != 0 && (char*)ExpEnv.debug_struc.pprint_me.nativerun_exit_by_fail != (char*)0x1) { \
                                         fprintf(stderr, "%s:%d\n", __FILE__, __LINE__); \
                                         fprintf(stderr, "%s", (char*)ExpEnv.debug_struc.pprint_me.nativerun_exit_by_fail); \
                                         } \
...
#define check_trail(x) if (__builtin_expect((Unsigned(CurrentTrailTop) < Unsigned(x)),0)) { \
    if ((char*)ExpEnv.debug_struc.pprint_me.native_treat_trail != 0 && (char*)ExpEnv.debug_struc.pprint_me.native_treat_trail != (char*)0x1) { \
      fprintf(stderr, "%s:%d\n", __FILE__, __LINE__);     \
      fprintf(stderr, "%s", (char*)ExpEnv.debug_struc.pprint_me.native_treat_trail); \
    }                 \
...
#define check_trail(x) if (__builtin_expect((Unsigned(CurrentTrailTop) < Unsigned(x)),0)) { \
    if ((char*)ExpEnv.debug_struc.pprint_me.interpreted_treat_trail != 0 && (char*)ExpEnv.debug_struc.pprint_me.interpreted_treat_trail != (char*)0x1) { \
      fprintf(stderr, "%s:%d\n", __FILE__, __LINE__);     \
      fprintf(stderr, "%s", (char*)ExpEnv.debug_struc.pprint_me.interpreted_treat_trail); \
    }                 \
    goto notrailleft;             \
  }
...
#define check_stack(Label, GLOB)          \
  if (__builtin_expect( ((Int)(Unsigned(YOUNGEST_CP((choiceptr)ENV_YREG,B_FZ)) - Unsigned(YOUNGEST_H(H_FZ,GLOB))) < CreepFlag), 0)  ) { \
    if ((char*)ExpEnv.debug_struc.pprint_me.interpreted_treat_heap != 0 && (char*)ExpEnv.debug_struc.pprint_me.interpreted_treat_heap != (char*)0x1) { \
      fprintf(stderr, "%s:%d\n", __FILE__, __LINE__);     \
      fprintf(stderr, "%s", (char*)ExpEnv.debug_struc.pprint_me.interpreted_treat_heap); \
    }                 \
    goto Label;               \
  }
#else
#define check_stack(Label, GLOB)          \
  if  (__builtin_expect(((Int)(Unsigned(ENV_YREG) - Unsigned(GLOB)) < CreepFlag ), 0) ) { \
    if ((char*)ExpEnv.debug_struc.pprint_me.interpreted_treat_heap != 0 && (char*)ExpEnv.debug_struc.pprint_me.interpreted_treat_heap != (char*)0x1) { \
      fprintf(stderr, "%s:%d\n", __FILE__, __LINE__);     \
      fprintf(stderr, "%s", (char*)ExpEnv.debug_struc.pprint_me.interpreted_treat_heap); \
    }                 \
...
#define restore_yaam_regs(AP)     \
  { register CELL *x1 = B_YREG->cp_env;   \
    ...
    x2 = (yamop *)AP;       \
    ENV    = x1;        \
    YAPOR_update_alternative((*_PREG), x2)  \
      B_YREG->cp_ap = x2;     \
  }
#endif
...
#define FAIL() {              \
    if ((char*)ExpEnv.debug_struc.pprint_me.interpreted_backtrack != 0 && (char*)ExpEnv.debug_struc.pprint_me.interpreted_backtrack != (char*)0x1) { \
      fprintf(stderr, "%s:%d\n", __FILE__, __LINE__);     \
      fprintf(stderr, "%s", (char*)ExpEnv.debug_struc.pprint_me.interpreted_backtrack); \
    }                 \
    goto fail;                \
  }

#define TRACED_FAIL() {             \
    if ((char*)ExpEnv.debug_struc.pprint_me.profiled_interpreted_backtrack != 0 && (char*)ExpEnv.debug_struc.pprint_me.profiled_interpreted_backtrack != (char*)0x1) { \
      fprintf(stderr, "%s:%d\n", __FILE__, __LINE__);     \
      fprintf(stderr, "%s", (char*)ExpEnv.debug_struc.pprint_me.profiled_interpreted_backtrack); \
    }                 \
    goto traced_fail;             \
  }
...
#define FAIL() { return external_labels[0]; }
...
#define traced_UnifyBound(d0,d1)          \
  ...
  if (IsPairTerm(d0)) {             \
    ...
    register CELL *ipt0, *ipt1;           \
    register Functor f;             \
    if (!IsApplTerm(d1)) { TRACED_FAIL(); }       \
    ipt0 = RepAppl(d0);             \
    ipt1 = RepAppl(d1);             \
    f = (Functor)*ipt0;             \
    if (f != (Functor)*ipt1) { TRACED_FAIL(); }       \
